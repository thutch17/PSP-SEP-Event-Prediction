...
name = f"SS_{wavelength}A_ep{epochs}_bs{batch_size}_lr{learning_rate}_dense{num_dense}_conv{num_conv}_drop{dropout_rate}_trainbatch{train_block_size}_seed{seed}"
print("training:", name)
...
for i, target_name in enumerate(["epilo"]):
    ...
    ax_train.set_xlabel(f"Actual {target_name}")
    ax_train.set_ylabel(f"Predicted {target_name}")
    ax_train.set_title(f"Train: Predicted vs Actual {target_name} (colored by date)")
    ...
    wandb.log({f"train_pred_vs_actual_{target_name}": wandb.Image(fig_train)})
    plt.close(fig_train)
    ...
    ax_val.set_xlabel(f"Actual {target_name}")
    ax_val.set_ylabel(f"Predicted {target_name}")
    ...
    cbar.ax.yaxis.set_major_locator(mdates.AutoDateLocator())
    cbar.ax.yaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))
    wandb.log({f"val_pred_vs_actual_{target_name}": wandb.Image(fig_val)})
    plt.close(fig_val)
